        ...
        <div class="order_details">
          <div class="card">
            <div class="card-header alert d-flex justify-content-between align-items-center">
              <h3>{{ __('adminlte::adminlte.deleted_users') }}</h3>
            ...
            <div class="card-body">
              ...
              <table style="width:100%" id="deleted-jobseekers-list" class="table table-bordered table-hover">
                ...
                <tbody>
                  <?php for ($i=0; $i < count(is_countable($users)?$users:[]); $i++) { ?>
                  <tr id="user_row_{{ $users[$i]->id }}">
                    ...
                    <td>{{ $users[$i]->phone_number }}</td>
                      ...
                      <td>
                      <a class="action-button restore-button" title="Restore" href="javascript:void(0)" data-id="{{ $users[$i]->id}}"><i class="text-success fa fa-undo"></i></a>
                      <a class="action-button remove-button" title="Permanent Delete" href="javascript:void(0)" data-id="{{ $users[$i]->id}}"><i class="text-danger fa fa-trash"></i></a>
                      <!-- @can('manage_recycle_bin_users') -->
                      <!-- @can('restore_user') -->
  ...
  <script>
    ...
    $('.remove-button').click(function(e) {
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{ route('permanent_delete_app_users') }}",
            type: 'post',
            data: {
              id: id
            },
