          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <table style="width:100%" id="deleted-jobseekers-list" class="table table-bordered table-hover">
                ...
                <tbody>
                  <?php for ($i=0; $i < count(is_countable($jobs)?$jobs:[]); $i++) { ?>
                  <tr id="user_row_{{ $jobs[$i]->id }}">
                    ...
                    <td>{{ $jobs[$i]->title }}</td>
                      ...
                      <td>
                      <a class="action-button restore-button" title="Restore" href="javascript:void(0)" data-id="{{ $jobs[$i]->id}}"><i class="text-success fa fa-undo"></i></a>
                      <a class="action-button remove-button" title="Permanent Delete" href="javascript:void(0)" data-id="{{ $jobs[$i]->id}}"><i class="text-danger fa fa-trash"></i></a>
                      <!-- @can('manage_recycle_bin_users') -->
                      <!-- @can('restore_user') -->
  ...
  <script>
    ...
    $('.remove-button').click(function(e) {
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{ route('permanent_delete_jobs') }}",
            type: 'post',
            data: {
              id: id
            },
