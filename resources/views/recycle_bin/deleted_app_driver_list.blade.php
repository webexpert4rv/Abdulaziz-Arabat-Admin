        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <table style="width:100%" id="deleted-jobseekers-list" class="table table-bordered table-hover">
              ...
              <tbody>
                <?php for ($i=0; $i < count(is_countable($drivers)?$drivers:[]); $i++) { ?>
                <tr id="user_row_{{ $drivers[$i]->id }}">
                  <td class="display-none"></td>
                  <td>{{ $drivers[$i]->name }}</td>
                  <td>{{ $drivers[$i]->phone_number }}</td>
            
                
                    <td>
                    <a class="action-button restore-button" title="Restore" href="javascript:void(0)" data-id="{{ $drivers[$i]->id}}"><i class="text-success fa fa-undo"></i></a>
                    <a class="action-button remove-button" title="Permanent Delete" href="javascript:void(0)" data-id="{{ $drivers[$i]->id}}"><i class="text-danger fa fa-trash"></i></a>
                    <!-- @can('manage_recycle_bin_users') -->
                    <!-- @can('restore_user') -->
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.min.js"></script>
  <script>
    ...
    $('.restore-button').click(function(e) {
      ...
      swal({
        title: "Are you sure?",
        text: "Are you sure you want to restore the Driver?",
        type: "warning",
    ...
    $('.remove-button').click(function(e) {
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{ route('permanent_delete_driver') }}",
            type: 'post',
            data: {
              id: id
            },
