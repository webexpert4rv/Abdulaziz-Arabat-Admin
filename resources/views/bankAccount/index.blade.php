          ...
          <div class="card">
            ...
            <div class="card-header alert d-flex justify-content-between align-items-center">
             <h3>{{ __('adminlte::adminlte.bank-account') }}</h3> 
              ...
              <a class="btn btn-sm btn-success" href="{{route('bank-account.create')}}">Add Bank Account</a>
            ...
            <div class="card-body">
              ...
              <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                <thead>
                  <tr>
                    <th >Sr.No.</th>
                    <th>{{ __('adminlte::adminlte.vendor_name') }}</th>
                    <th>{{ __('adminlte::adminlte.bank_name') }}</th>
                    <th>{{ __('adminlte::adminlte.account_number') }}</th>
                    <th>{{ __('adminlte::adminlte.iban_no') }}</th>
                  ...
                  <tbody id="tbody">
                  @foreach($getData as $key=>$product)
                    <tr class="remove_product{{$product->id}}">
                        <td >{{$key+1}}</td>
                        <td>{{$product->vendor_name}}</td>
                        <td>{{$product->bank_name}}</td>
                        <td>{{$product->account_number}}</td>
                        <td>{{$product->iban_no}}</td>
                        <td>
                        <label class="switch">
                            <input type="checkbox" onchange="updateStatus({{$product->id}})" type="checkbox" id="demo{{$product->id}}" {{$product->status==1?'checked':''}}>
                            <span class="slider round"></span>
                        </label>
                        </td>
                      
                        <td>
                            <a class="action-button" title="Edit" href="{{route('bank-account.edit',$product->id)}}"><i class="text-warning fa fa-edit"></i></a>
...
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>
  ...
  <script type="text/javascript">
     function productDelete(id){
      ...
      swal({
        title: "Are you sure?",
        text: "Are you sure you want to delete the bank account details?",
        type: "warning",
  ...
  function updateStatus(id){ 
    ...
    $.ajax({
        type: 'POST',
        url: "{{route('update.bankaccount.status')}}",
        data: {
            "_token"      : "{{ csrf_token() }}",
            "status"      : status,
            "id"          : id,
