        ...
        <div class="order_details">
          <div class="card">
            ...
            <div class="card-header alert d-flex justify-content-between align-items-center">
              <h3>{{ __('adminlte::adminlte.regions') }}</h3>
            
              <a class="btn btn-sm btn-success" href="{{route('region.create')}}">{{ __('adminlte::adminlte.add_region') }}</a>
                ...
                <thead>
                  ...
                  @foreach($regions as $key=>$region)
                    <tr class="remove_region{{$region->id}}">
                        ...
                        <td>
                        <label class="switch">
                            <input type="checkbox" onchange="updateStatus({{$region->id}})" type="checkbox" id="demo{{$region->id}}" {{$region->status==1?'checked':''}}>
                            <span class="slider round"></span>
                        </label>
                        </td>
                        <td>

                            <a class="action-button" title="Edit" href="{{route('region.edit',$region->id)}}"><i class="text-warning fa fa-edit"></i></a>
...
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>
  ...
  function updateStatus(id){ 
    ...
    $.ajax({
        type: 'POST',
        url: "{{route('update.region.status')}}",
        data: {
            "_token"      : "{{ csrf_token() }}",
            "status"      : status,
            "id"          : id,
