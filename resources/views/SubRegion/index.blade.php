        ...
        <div class="order_details">
          <div class="card">
            ...
            <div class="card-header alert d-flex justify-content-between align-items-center">
              <h3>{{ __('adminlte::adminlte.sub_regions') }}</h3>
            
              <a class="btn btn-sm btn-success" href="{{route('sub-region.create')}}">{{ __('adminlte::adminlte.add_sub_region') }}</a>
            ...
            <div class="card-body">
              ...
              <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                <thead>
                  <tr>
                    <th >Sr.No.</th>
                    <th>{{ __('adminlte::adminlte.region') }}</th>
                    <th>{{ __('adminlte::adminlte.subregion') }}</th>
                  ...
                  @foreach($subRegions as $key=>$region)
                    <tr class="remove_region{{$region->id}}">
                        <td >{{$key+1}}</td>
                        <td>{{@$region->Region->name}}({{@$region->Region->arabic_name}})</td>
                        <td>{{@$region->name}}</td>
                        <td>{{@$region->arabic_name}}</td>
                         <td>
                        <label class="switch">
                            <input type="checkbox" onchange="updateSubRegionStatus({{$region->id}})" type="checkbox" id="demo{{$region->id}}" {{$region->status==1?'checked':''}}>
                            <span class="slider round"></span>
                        </label>
                        </td>
                      
                        <td>
                            <a class="action-button" title="Edit" href="{{route('sub-region.edit',$region->id)}}"><i class="text-warning fa fa-edit"></i></a>
                      
                            <a class="action-button delete-button" title="Delete" onclick="regionDelete({{$region->id}});" href="javascript:void(0)" data-id=""><i class="text-danger fa fa-trash-alt"></i></a>
...
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>
  ...
  <script type="text/javascript">
     function regionDelete(id){
      ...
      swal({
        title: "Are you sure?",
        text: "Are you sure you want to delete the sub region?",
        type: "warning",
  ...
  <script>
    ...
    function updateSubRegionStatus(id){ 
    ...
    $.ajax({
        type: 'POST',
        url: "{{route('update.sub-region.status')}}",
        data: {
            "_token"      : "{{ csrf_token() }}",
            "status"      : status,
            "id"          : id,
